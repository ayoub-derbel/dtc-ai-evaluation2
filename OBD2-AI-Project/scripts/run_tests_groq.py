#!/usr/bin/env python3
"""Run OBD-II prompt benchmark with Groq model llama-3.3-70b-versatile."""
...
def export_pdf(results: list[dict[str, str]], pdf_path: Path) -> None:
    ...
    pdf.drawString(x_margin, y, "OBD2 AI Benchmark Report - Groq")
    y -= 24
...
def main() -> None:
    if not GROQ_API_KEY or GROQ_API_KEY == "REPLACE_WITH_YOUR_GROQ_API_KEY":
        raise RuntimeError("Set GROQ_API_KEY directly in scripts/run_tests_groq.py before running tests.")
    ...
    results_path = project_root / "results" / "responses_groq.json"
    pdf_path = project_root / "results" / "responses_groq_report.pdf"
