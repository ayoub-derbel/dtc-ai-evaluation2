#!/usr/bin/env python3
"""Run OBD-II prompt benchmark with GPT-4o via OpenRouter."""
...
def export_pdf(results: list[dict[str, str]], pdf_path: Path) -> None:
    ...
    pdf.drawString(x_margin, y, "OBD2 AI Benchmark Report - GPT-4o")
    y -= 24
...
def main() -> None:
    if not OPENROUTER_API_KEY or OPENROUTER_API_KEY == "REPLACE_WITH_YOUR_OPENROUTER_API_KEY":
        raise RuntimeError("Set OPENROUTER_API_KEY directly in scripts/run_tests_gpt.py before running tests.")
    ...
    results_path = project_root / "results" / "responses_gpt.json"
    pdf_path = project_root / "results" / "responses_gpt_report.pdf"
