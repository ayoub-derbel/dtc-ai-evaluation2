#!/usr/bin/env python3
"""Run OBD-II prompt benchmark with Groq model deepseek-r1-distill-llama-70b."""
...
MODEL_LABEL = "deepseek-r1-distill-llama-70b"
...
def ask_continue_or_stop() -> bool:
    ...
    print("2) Stop now and generate outputs with current results")
    while True:
...
def export_pdf(results: list[dict[str, str]], pdf_path: Path) -> None:
    ...
    pdf.drawString(x_margin, y, "OBD2 AI Benchmark Report - Groq DeepSeek Distill")
    y -= 24
...
def main() -> None:
    if not GROQ_API_KEY or GROQ_API_KEY == "REPLACE_WITH_YOUR_GROQ_API_KEY":
        raise RuntimeError(
            "Set GROQ_API_KEY directly in scripts/run_tests_groq_deepseek_distill.py before running tests."
        )

    project_root = Path(__file__).resolve().parents[1]
    prompts = load_json(project_root / "prompts" / "prompts.json").get("prompts", [])
    tests = load_json(project_root / "dataset" / "dtc_tests.json").get("tests", [])
    results_path = project_root / "results" / "responses_groq_deepseek_distill.json"
    pdf_path = project_root / "results" / "responses_groq_deepseek_distill_report.pdf"
    ...
    for prompt_item in prompts:
        ...
        for test_case in tests:
            ...
            try:
                response_text = call_groq(GROQ_API_KEY, prompt_text)
