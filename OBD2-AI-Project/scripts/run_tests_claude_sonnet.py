#!/usr/bin/env python3
"""Run OBD-II prompt benchmark with OpenRouter model anthropic/claude-sonnet-4.5."""
...
def call_openrouter(api_key: str, prompt: str) -> str:
    headers = {
        ...
        "X-Title": "OBD2-AI-Test-Claude-Sonnet",
    }
    payload = {
        "model": API_MODEL,
...
def export_pdf(results: list[dict[str, str]], pdf_path: Path) -> None:
    ...
    pdf.drawString(x_margin, y, "OBD2 AI Benchmark Report - Claude Sonnet 4.5")
    y -= 24
...
def main() -> None:
    if not OPENROUTER_API_KEY or OPENROUTER_API_KEY == "REPLACE_WITH_YOUR_OPENROUTER_API_KEY":
        raise RuntimeError("Set OPENROUTER_API_KEY directly in scripts/run_tests_claude_sonnet.py before running tests.")
    ...
    results_path = project_root / "results" / "responses_claude_sonnet.json"
    pdf_path = project_root / "results" / "responses_claude_sonnet_report.pdf"
